...
public class DefaultMySQLTenantPermissionStore implements TenantPermissionStore {
	...
	private final static Log log = LogFactory
			.getLog(DefaultMySQLTenantPermissionStore.class);
	private DataSource dataSource;

	public DefaultMySQLTenantPermissionStore() {
		try {
			this.dataSource = Utils
					.getDataSource(Configuration.PERMISSION_STORE_DS);
		} catch (Exception e) {
		...
		try {
			...
			if (tenantId == MultitenantConstants.INVALID_TENANT_ID) {
				throw new PermissionStoreException(
						"tenant name is not in the System ");
			}
			...
			throw new PermissionStoreException(
					"Cannot add tenant subscription to permission " + "store ",
					e);
		} finally {
	...
	public void unsubscribeTenant(String tenant, String permissionGroupID)
		...
		try {
			...
			statement = connection
					.prepareStatement(Constants.UNSUBSCRIBE_TENANT);
			statement.setInt(1, Utils.getTenantId(tenant));
			statement.setString(2, permissionGroupID);
			statement.executeUpdate();
		} catch (Exception e) {
			throw new PermissionStoreException(
					"Cannot unsubscribe tenant from permission " + "store ", e);
		} finally {
			DatabaseUtil.closeAllConnections(connection, statement);
		}
	}
	...
	@Override
	public Subscription getTenantSubscription(String tenantDomain)
			throws PermissionStoreException {
		Connection connection = null;
		PreparedStatement statement = null;
		...
		try {
			connection = dataSource.getConnection();
			statement = connection
					.prepareStatement(Constants.GET_TENANT_SUBSCRIPTIONS);
			...
			throw new PermissionStoreException(
					"Cannot get tenant subscription from permission "
							+ "store ", e);
		} finally {
	...
	@Override
	public void deleteTenantSubscription(String tenant)
			throws PermissionStoreException {
		Connection connection = null;
		PreparedStatement statement = null;
		try {
			connection = dataSource.getConnection();
			statement = connection
					.prepareStatement(Constants.REMOVE_TENANT_SUBSCRIPTION);
			statement.setInt(1, Utils.getTenantId(tenant));
			statement.executeUpdate();
		} catch (Exception e) {
			throw new PermissionStoreException(
					"Cannot delete tenant subscription from "
							+ "permission store ", e);
		} finally {
			DatabaseUtil.closeAllConnections(connection, statement);
		}
	...
	public void addPermissionGroup(PermissionGroup permissionGroup)
		...
		Boolean previousAutocommitStatus = null;
		try {
			connection = dataSource.getConnection();
			...
			try {
				...
				log.error(
						"Unable to rollback last insert statement of the permission set on permission group ID: "
								+ permissionGroup.getId(), e);
			}
			throw new PermissionStoreException(
					"Cannot add permission group to permission store ", e);
		} finally {
			if (previousAutocommitStatus != null) {
				try {
					connection.setAutoCommit(previousAutocommitStatus);
				} catch (SQLException e) {
					log.error(
							"Unable to reset the autoCommit status of the connection",
							e);
				}
			}
			DatabaseUtil.closeAllConnections(connection, statement);
	...
	public PermissionGroup getPermissionGroup(String groupName)
		...
		try {
			connection = dataSource.getConnection();
			statement = connection
					.prepareStatement(Constants.GET_PERMISSION_GROUP);
			...
			throw new PermissionStoreException(
					"Cannot get permission group from permission store ", e);
		} finally {
			DatabaseUtil.closeAllConnections(connection, resultSet, statement);
		}
	}
	...
	public void deletePermissionGroup(String groupName)
		...
		try {
			connection = dataSource.getConnection();
			statement = connection
					.prepareStatement(Constants.DELETE_PERMISSION_GROUP);
			statement.setString(1, groupName);
			statement.executeUpdate();
		} catch (SQLException e) {
			throw new PermissionStoreException(
					"Cannot delete permission group from permission "
							+ "store ", e);
		} finally {
			DatabaseUtil.closeAllConnections(connection, statement);
		}
