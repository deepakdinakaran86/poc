...
public enum TagErrorCodes implements GalaxyErrorCode {
	...
	private TagErrorCodes(Integer errorCode, String message, String description) {
		this.code = errorCode;
	...
	private static final Map<Integer, TagErrorCodes> map;
	static {
		map = new HashMap<Integer, TagErrorCodes>();
		for (TagErrorCodes v : TagErrorCodes.values()) {
			map.put(v.getCode(), v);
		}
	}

	public static TagErrorCodes getErrorCode(Integer errorCode) {
		TagErrorCodes result = map.get(errorCode);
		if (result == null) {
