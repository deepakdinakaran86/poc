 ...
 * DERIVATIVES.
 */
package com.pcs.alpine.rest.client;

import static com.pcs.alpine.rest.exception.GalaxyRestErrorCodes.UN_MARSHAL_ERROR;
...
import com.pcs.alpine.commons.rest.dto.ErrorMessageISDTO;
import com.pcs.alpine.constant.CommonConstants;
import com.pcs.alpine.rest.beans.StandardResponse;
import com.pcs.alpine.rest.exception.GalaxyRestErrorCodes;
import com.pcs.alpine.rest.exception.GalaxyRestException;
import com.pcs.alpine.rest.mapper.GalaxyObjectMapper;
	...
	public <R> R handleResponse(HttpResponse httpResponse,
		...
		try {
			...
			if (statusCode == HttpStatus.SC_OK) {
				...
				try {
					// StandardResponse errorStatus = mapper.readValue(
					// httpResponse.getEntity().getContent(),
					// StandardResponse.class);
					// throw new GalaxyRestException(
					// GalaxyRestErrorCodes.INVALID_REQUEST,
					// errorStatus.getMessage());
					ErrorMessageISDTO errorMessageISDTO = unmarshall(
							ErrorMessageISDTO.class, httpResponse.getEntity()
									.getContent());
					if (errorMessageISDTO.getStatus() == null
							&& errorMessageISDTO.getMessage() == null) {
						ErrorMessageDTO errorMessageDTO = new ErrorMessageDTO();
						errorMessageDTO.setErrorCode(errorMessageISDTO
								.getErrorCode());
						errorMessageDTO.setErrorMessage(errorMessageISDTO
								.getErrorMessage());
						throw new GalaxyRestException(errorMessageDTO);
					} else {
						ErrorMessageDTO errorMessageDTO = new ErrorMessageDTO();
						errorMessageDTO
								.setErrorCode(String.valueOf(GalaxyCommonErrorCodes.SPECIFIC_DATA_NOT_AVAILABLE.getCode()));
						errorMessageDTO.setErrorMessage(errorMessageISDTO
								.getMessage());
						throw new GalaxyRestException(errorMessageDTO);
					 ...
					 * .getContent()); throw new
					 * GalaxyRestException(errorMessageDTO);
					 */
