		...
		$.ajax({
			...
			success : function(response) {
				...
				if (response.entity != null) {
					...
					var deviceSelected = '${generator_form.device}';
					if (deviceSelected != '') {
...
<div style="height: 1px;">
	<form id="generatorHome" action="generator_home" method="get"></form>
</div>

<form:form role="form" action="generator_create" commandName="generator_form"
	id="generator_form" autocomplete="off" novalidate="novalidate">
...
<div>
	<button class="button text-shadow large-button fg-white bg-blue "
		style="height: 40px; width: 80px;" id="generator-btn-save"
...
<script type="text/javascript">
	...
	if ('${action}' === 'View') {
		$("#generator_form :input").prop("disabled", true);
		$("#generator-btn-save").hide();
	} else {
		setFormValidation();
		entityName = '${generator_form.entityName}';
		esn = '${generator_form.esn}';
	}
