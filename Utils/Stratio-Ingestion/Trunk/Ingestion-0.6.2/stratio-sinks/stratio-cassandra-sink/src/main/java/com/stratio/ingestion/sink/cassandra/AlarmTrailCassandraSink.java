...
public class AlarmTrailCassandraSink extends AbstractSink implements
		Configurable {

	private static final Logger log = LoggerFactory
			.getLogger(AlarmTrailCassandraSink.class);
	...
	public AlarmTrailCassandraSink() {
		super();
	}
		...
		try {
			...
			for (count = 0; count < batchSize; ++count) {
				...
				JsonObject json = parser.parse(new String(event.getBody()))
						.getAsJsonObject();
				json.remove("receivedTime");

				json.addProperty("device", json.get("sourceId").getAsString());
				json.remove("sourceId");
				json.addProperty("alarmtime", json.get("time").getAsString());
				json.remove("time");
				json.addProperty("alarmstate", json.get("status").getAsString());
				json.remove("status");
				
				json.remove("points");
				
				long insertTimeMs = new DateTime(DateTimeZone.UTC).getMillis();
				json.addProperty("inserttime", insertTimeMs);
				long alarmDateStartOfDay = new DateTime(Long.parseLong(json.get("alarmtime").getAsString()))
						.withTimeAtStartOfDay().getMillis();
				json.addProperty("date", alarmDateStartOfDay);

				Statement statement = new SimpleStatement("insert into alarm_trail "+
						 " JSON " + "'"
	...
	public static void main(String[] args) {
		...
		String event = "{ \"sourceId\": \"259642460475435\", \"time\": \"1460248776000\", \"receivedTime\": \"1460248721456\", \"alarmMessage\": \"AO1_LowlimitAlarmActive\", \"alarmName\": \"OUT OF RANGE ALARM\", \"alarmType\": \"OUT OF RANGE ALARM\", \"status\": \"true\", \"displayName\": \"Aftertreatment 1 Diesel Exhaust Fluid Tank Level-12\", \"data\": \"9.9994545\", \"unit\": \"unitless\", \"points\": [ { \"pointId\": \"12\", \"pointName\": \"Aftertreatment 1 Diesel Exhaust Fluid Tank Level\", \"displayName\": \"Aftertreatment 1 Diesel Exhaust Fluid Tank Level-12\", \"unit\": \"unitless\", \"data\": \"9.9994545\", \"status\": \"null\", \"type\": \"FLOAT\", \"extensions\": [ { \"extensionName\": \"WRITEABLE\", \"extensionType\": \"ACCESS TYPE\" }, { \"extensionName\": \"STATECHANGE ACQUISITION\", \"extensionType\": \"Acquisition Mode\" } ] } ] }";
		JsonObject json = parser.parse(event).getAsJsonObject();
		json.remove("receivedTime");
		...
		json.addProperty("alarmstate", json.get("status").getAsString());
		json.remove("status");
		//json.add("alarmpoints", json.get("points").getAsJsonArray());
		
		json.remove("points");

		System.out.println("For alarm " + json.toString());
	}
