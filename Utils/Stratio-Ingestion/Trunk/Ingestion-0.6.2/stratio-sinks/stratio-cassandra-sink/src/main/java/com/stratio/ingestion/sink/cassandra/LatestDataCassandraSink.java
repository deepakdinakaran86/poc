...
public class LatestDataCassandraSink extends AbstractSink implements
		Configurable {

	private static final Logger log = LoggerFactory
			.getLogger(LatestDataCassandraSink.class);
	...
	public LatestDataCassandraSink() {
		super();
	}
	...
	public Status process() throws EventDeliveryException {
		...
		try {
			...
			for (count = 0; count < batchSize; ++count) {
				...
				List<IngestionLatestData> latestDataMessages = mapper.readValue(event.getBody(),new TypeReference<List<IngestionLatestData>>() {
				});
				if(!CollectionUtils.isEmpty(latestDataMessages)){
					for (IngestionLatestData latestDataMessage : latestDataMessages) {
						latestDataMessage.setData(mapper.writeValueAsString(latestDataMessage.getData()));
						String ingestionJSON = mapper.writeValueAsString(latestDataMessage);
						
						Statement statement = new SimpleStatement("insert into latestdata JSON " + "'"
