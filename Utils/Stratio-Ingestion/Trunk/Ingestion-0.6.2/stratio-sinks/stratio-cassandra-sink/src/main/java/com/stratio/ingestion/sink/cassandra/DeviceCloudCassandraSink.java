...
import com.stratio.ingestion.sink.cassandra.bean.IngestionData;
import com.stratio.ingestion.sink.cassandra.bean.IngestionDetails;
import com.stratio.ingestion.sink.cassandra.bean.IngestionLatestData;

public class DeviceCloudCassandraSink extends AbstractSink implements
		Configurable {
	...
	public DeviceCloudCassandraSink() {
		super();
	}
	...
	public Status process() throws EventDeliveryException {
		...
		try {
			...
			for (count = 0; count < batchSize; ++count) {
				...
				IngestionDetails ingestionDetails = mapper.readValue(new String(event.getBody()), IngestionDetails.class);
				IngestionData ingestionData = new IngestionData();

				ingestionData.setDevice(ingestionDetails.getDeviceId());
				ingestionData.setDate(ingestionDetails.getDeviceDate());
				ingestionData.setDeviceTime(ingestionDetails.getDeviceTime());
				ingestionData.setInsertTime(ingestionDetails.getInsertTime());
				ingestionData.setExtension(ingestionDetails.getExtensions());
				ingestionData.setData(ingestionDetails.getData());
				ingestionData.setDisplayName(ingestionDetails.getDisplayName());
				ingestionData.setSystemTag(ingestionDetails.getSystemTag());

				String ingestionJSON = mapper.writeValueAsString(ingestionData);
				

				Statement statement = new SimpleStatement("insert into "
						+ ingestionDetails.getDatastore() + " JSON " + "'"
						+ ingestionJSON.toString().replaceAll("'", "''") + "'");
				long startTime = Calendar.getInstance().getTimeInMillis();
				
				log.info("Inserting data -> {}",statement.toString());
				this.session.execute(statement);
				log.info("Insertion completed, took {} millis", Calendar.getInstance().getTimeInMillis() - startTime);
	...
	public static void main(String[] args) throws JsonGenerationException, JsonMappingException, IOException {
		String event = "{\"deviceId\":\"4fd4266e-d4df-4809-8711-36c31777949f\", \"deviceTime\":1467530971732, \"deviceDate\":1467504000000, \"insertTime\":1467530971732, \"displayName\":\"Location\", \"data\":{\"latitude\":24.93925,\"longitude\":55.052025}, \"datastore\":\"location\", \"extensions\":[], \"dataType\":\"GEOPOINT\", \"isLatest\":true, \"systemTag\":\"\"}";

		ObjectMapper mapper = new ObjectMapper();
		IngestionDetails ingestionDetails = mapper.readValue(event, IngestionDetails.class);
		IngestionData ingestionData = new IngestionData();
