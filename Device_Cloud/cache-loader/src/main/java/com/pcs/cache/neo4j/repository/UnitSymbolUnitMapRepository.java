...
/**
 * Unit name stored against unit symbol
 * 
 * @author pcseg199(Javid Ahammed)
 * @date 21 Dec, 2015
 * @since galaxy-1.0.0
 */
public class UnitSymbolUnitMapRepository implements MapStoreRepoI<String, String> {
	...
	private static String getAllUnitSymbols = "MATCH (n:UNIT) where n.symbol <> '' return {symbol:n.symbol}";
	private static String getUnitOfSymbol = "MATCH (n:UNIT ) WHERE n.symbol ='{symbol}' OR n.symbol ='{symbol_encoded}' return {name:n.name};";
	...
	public String getValueOfkey(String key) {
		...
		try {
			String query = getUnitOfSymbol.replace("{symbol}", key).replace(
