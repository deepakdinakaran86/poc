...
public class AnalyticsUtil {
	...
	private static final String KEYSEPERATOR = "_";
	...
	private static void emptyCheck(Object parameter) {
	...
	public static Long getDatasourcePublishTime(String sourceId,String displayName) {
		emptyCheck(sourceId);
		emptyCheck(displayName);
		String cacheKey = sourceId+KEYSEPERATOR+displayName;
		Cache deviceCache = ApplicationConfig.getCacheProvider().getCache(DEVICE_DATASOURCE_CACHE);
		Long lastPublishTime = deviceCache.get(cacheKey,Long.class);
		return lastPublishTime;
	}
	
	public static boolean updateDatasourcePublishTime(String sourceId,String displayName,long messageTime) {
		emptyCheck(sourceId);
		emptyCheck(displayName);
		String cacheKey = sourceId+KEYSEPERATOR+displayName;
		Cache deviceCache = ApplicationConfig.getCacheProvider().getCache(DEVICE_DATASOURCE_CACHE);
		deviceCache.put(cacheKey, messageTime);
		return true;
	}
	...
	private static DeviceAuthenticationResponse getDeviceFromStore(
		...
		try {
			...
			device.setSourceId(sourceId.toString());
			authnResponse = httpClient.post(url, null,device,
