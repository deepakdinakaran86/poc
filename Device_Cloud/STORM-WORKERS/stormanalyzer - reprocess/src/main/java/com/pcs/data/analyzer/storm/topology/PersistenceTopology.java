...
import com.pcs.data.analyzer.storm.bolts.EventCandidateProvider;
...
public class PersistenceTopology {
	...
	private static String eventCandidateProvider="event-candidate-bolt";
	...
	private static int eventCandidateBoltMaxParallelism;
	...
	private static void initialize() {
		try {
			...
			ApplicationConfig.getCacheProvider();
			...
			eventCandidateProvider = load.get(Configuration.EVENT_CANDIDATE_BOLT).toString();
	        ...
	        Object eventCandidateBoltMaxParallelismObj = load
			        .get(Configuration.PERSIST_BOLT_MAX_PARALLELISM);

			eventCandidateBoltMaxParallelism = eventCandidateBoltMaxParallelismObj != null
			        ? Integer.parseInt(eventCandidateBoltMaxParallelismObj.toString())
			        : 1;
	...
	private static StormTopology buildTopology() {
		...
		builder.setSpout(parentSpoutName, buildKafkaSpout(parentSpoutName),
		...
		builder.setBolt(eventCandidateProvider, new EventCandidateProvider(publisherConfFilePath,"galaxy_input_queue"),
				eventCandidateBoltMaxParallelism).shuffleGrouping(parentSpoutName);
	 ...
	 * @return
	 */
	private static KafkaSpout buildKafkaSpout(String spoutName) {
		...
		kafkaConfig.scheme = new SchemeAsMultiScheme(new MessageScheme(spoutName));
