...
public class PointDiscoveryProcessor extends G2021Processor {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(PointDiscoveryProcessor.class);
	...
	public PointDiscoveryProcessor(Integer unitId) throws Exception {
		this.unitId = unitId;
		configuration = G2021DiagnosticHandler.deviceConfiguration;
	...
	public Message processG2021Message(Object G2021Data) throws Exception {
		...
		try {
			...
			for (int i = 0; i < totalPoints; i++) {
				...
				int offNormalLowerThresholdTextLength = pointData
						.readUnsignedByte();
				...
				for (int j = 0; j < offNormalLowerThresholdTextLength; j++) {
					offNormalLowerThresholdTextArray[j] = (char) pointData
							.readUnsignedByte();
				}
				String lowerThresholdAlarmMessage = new String(
						offNormalLowerThresholdTextArray);

				int offNormalUpperThresholdTextLength = pointData
						.readUnsignedByte();
				...
				for (int j = 0; j < offNormalUpperThresholdTextLength; j++) {
					offNormalUpperThresholdTextArray[j] = (char) pointData
							.readUnsignedByte();
				}
				String upperThresholdAlarmMessage = new String(
						offNormalUpperThresholdTextArray);

				Point point = Point.getPoint(resolvePointType(pointDataType)
						.getType());
				point.setPointName(pointName);
				point.setCustomTag(pointName);
				point.setUnit(unitName);
				point.setPhyQtyName(resolvePhysicalQuantity(unitName));
				point.setPointId(pointId.toString());
				LOGGER.info("Point created {},{},{}",point.getPointName(),point.getDataType(),point.getUnit());

				// Managing extensions
				point.setPointAccessType(resolveAccess(pointType));
				PointExtension acquisitionMode = getAcquisitionMode(pointAcqType,
						pointAcqTime, pointAcqCovThreshold);
				...
				if (alarmExtension != null) {
					if (alarmExtension instanceof StateChangeAlarm) {
						...
						outOfRangeAlarm
								.setLowerThreshold(pointAlarmLowerThreshold);
						outOfRangeAlarm
								.setLowerThresholdAlarmMessage(lowerThresholdAlarmMessage);
						outOfRangeAlarm
								.setLowerThresholdNormalMessage(normalMessage);
						outOfRangeAlarm
								.setUpperThreshold(pointAlarmUpperThreshold);
						outOfRangeAlarm
								.setUpperThresholdAlarmMessage(upperThresholdAlarmMessage);
						outOfRangeAlarm
								.setUpperThresholdNormalMessage(normalMessage);
	...
	private PointExtension getAcquisitionMode(int type, Short pointAcqTime,
			Float pointAcqCovThreshold) {
		switch (type) {
		case 0:
			return null;
		case 1:
			return new PointExtension("TIMEBASED @ " + pointAcqTime,
					"Acquisition Mode");
		case 2:
			return new PointExtension("STATECHANGE", "Acquisition Mode");
		case 3:
			return new PointExtension("COV @ " + pointAcqCovThreshold,
					"Acquisition Mode");
	...
	private String resolvePhysicalQuantity(String unitName) {
	...
	public static void main(String[] args) {
		byte[] hexStringToByteArray = (byte[]) ConversionUtils.hexStringToByteArray("03002A03414F32014B0104000000000000000000000000000000000000000000002B03414F3403C2B0460104000000000000000000000000000000000000000000002C03414F3603C2B0430104000000000000000000000000000000000000000000");
		ByteBuf buf = Unpooled.wrappedBuffer(hexStringToByteArray);
		try {
			new PointDiscoveryProcessor().processG2021Message(buf);
		} catch (Exception e) {
