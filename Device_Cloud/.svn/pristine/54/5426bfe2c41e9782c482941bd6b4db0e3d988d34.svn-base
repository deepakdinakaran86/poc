...
import com.pcs.data.analyzer.storm.persistence.util.PersistService;
...
public class AlarmPersistBolt extends BaseBasicBolt{
	...
	public void persistAlarm(AlarmMessage alarmMessage){
		try {
			this.persistService = new PersistService(publisherConfig);
			persistService.persist(alarmMessage);
