...
/**
 * This class is responsible for Longitude extraction
 * 
 * @author pcseg129 (Seena Jyothish)
 * @date March 31 2015
 */
public class Longitude_Extractor extends IOExtractor {

	private static Longitude_Extractor extractor = null;
	public static final long MILLIONTH = 1000000;

	private Longitude_Extractor(){
	}

	public static Longitude_Extractor getInstance(){
		if(extractor == null){
			extractor = new Longitude_Extractor();
		}
		return extractor;
	}
	...
	public Point getSpecificData(Point point) throws Exception {
		...
		String bitVarification = (String)ConversionUtils.convertToBinary(dataForVerification[0]);
		if(bitVarification.charAt(0)=='1'){
			Integer twosComplementData = (Integer)ConversionUtils.getTwosComplement(pointData);
			point.setData(twosComplementData.doubleValue()/(MILLIONTH*10));
		}else{
			String lng = (String)ConversionUtils.convertToLong(pointData);
