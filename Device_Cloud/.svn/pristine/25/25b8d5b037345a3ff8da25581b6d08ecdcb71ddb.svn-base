package com.pcs.device.gateway.G2021.message.header;
...
import com.pcs.device.gateway.G2021.exception.InvalidHeaderException;
	...
	private Header(ByteBuf msgData) throws InvalidHeaderException{
		...
		if(msgData != null && msgData.readableBytes() <3){
			...
			if(packetType == PacketType.ANONYMOUS_WITH_ASSURED_DELIVERY || 
				...
				Long unsignedSessionId = msgData.readUnsignedInt();
				sessionId = unsignedSessionId.intValue();
				Long unsignedUnitId = msgData.readUnsignedInt(); 
				unitId = unsignedUnitId.intValue();
				LOGGER.info("Session id : "+sessionId+" Unit Id : "+unitId);
				reqAck = true;
			}
		}
	}
