...
/**
 * Service Implementation for Genset Related APIs
 * 
 ...
 */
@Service
public class GensetServiceImpl implements EquipmentService {
	...
	public EntityDTO createEquipment(EquipmentDTO equipmentDTO) {
		...
		equipmentEntity = platformESBClient.post(
				...
				getJwtToken(),
				createAlpineEquipment(equipmentDTO, equipmentDTO.getEquipment()
		...
		if (equipmentDTO.getSkySparkEquipmentTags() != null) {
	...
	public StatusMessageDTO updateEquipment(EquipmentDTO equipmentDTO) {
		...
		if (isNotEmpty(pointsList)) {
			...
			skySparkEquipmentTags.put(POINTS.getVariableName(),
					pointsToSkySpark);
			...
			for (SkySparkPoint skySparkPoint : pointsList) {
				...
				try {
					...
					if (pointEntity != null) {
						...
						pointSkySparkMap.put(WebConstants.POINT_ID, pointId);
						if (StringUtils.isNotEmpty(tags)) {
							String[] tagsArray = tags.split(WebConstants.COMMA);
							if (tagsArray.length > 0) {
								for (String tag : tagsArray) {
									pointSkySparkMap.put(tag, true);
								}
							}
						}
	...
	public EquipmentDTO getEquipmentDetails(IdentityDTO identityDTO) {
		...
		EquipmentDTO avocadoEquipmentDTO = new EquipmentDTO();
		avocadoEquipmentDTO.setEquipment(identityDTO);
		avocadoEquipmentDTO.setFieldValues(equipmentEntity.getFieldValues());
		avocadoEquipmentDTO.setTree(equipmentEntity.getTree());
		...
		try {
			...
			if (isNotEmpty(points)) {
				avocadoEquipmentDTO.setPoints(points);
			}
		} catch (GalaxyRestException e) {
			LOGGER.info("No Points found for the Equipment");
		}
		return avocadoEquipmentDTO;
	}

	private void validateGensetIdentifier(IdentityDTO identityDTO) {
		validateEquipIdentifier(identityDTO);
		...
		if (identityDTO.getDomain() == null
			...
			contextDomain.setDomainName(getContext().getSubscription()
					.getEndUserDomain());
			identityDTO.setDomain(contextDomain);
		}
	}
	...
	public List<LatestDataResultDTO> getGensetLatestData(IdentityDTO identityDTO) {
		...
		validateGensetIdentifier(identityDTO);
		HierarchySearchDTO hierarchySearchDTO = new HierarchySearchDTO();
		hierarchySearchDTO.setActor(identityDTO);
		hierarchySearchDTO.setSearchEntityType(MARKER);
		hierarchySearchDTO.setSearchTemplateName(POINT_TEMPLATE);
	 ...
	 * (com.pcs.avocado.dto.PointRelationship)
	 */
	@Override
	public StatusMessageDTO createPointsAndRelationship(
		...
		// List<SkySparkPointCreate> skysparkPointList = new
		// ArrayList<SkySparkPointCreate>();
	...
	private List<EntityDTO> getDevicePoints(
		...
		try {
			...
			LOGGER.info("No points found from alpine");
			if (galaxyException.getCode() == GalaxyCommonErrorCodes.DATA_NOT_AVAILABLE.getCode()) {
				pointRelationship.setPointsExist(false);
			}
		}
