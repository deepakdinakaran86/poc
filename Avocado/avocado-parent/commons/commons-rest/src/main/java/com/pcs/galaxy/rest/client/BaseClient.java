...
import com.pcs.galaxy.rest.exception.GalaxyRestErrorCodes;
...
public class BaseClient {
	...
	private <R> R execute(HttpRequestBase method, String serviceContext,
		...
		HttpResponse httpResponse = connectionManager.execute(method,
		        serviceContext);
		return handleResponse(httpResponse, responseClazz);
	...
	public <R> R handleResponse(HttpResponse httpResponse,
		...
		try {
			...
			if (statusCode == HttpStatus.SC_OK) {
				R response = unmarshall(responseClazz, httpResponse.getEntity()
				        .getContent());
				return response;
