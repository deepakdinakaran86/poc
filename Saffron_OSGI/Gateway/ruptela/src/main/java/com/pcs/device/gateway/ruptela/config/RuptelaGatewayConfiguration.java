 ...
 *
 */
public class RuptelaGatewayConfiguration implements PropertyKeys {

	private static Properties props;
	private static final String configFileName = "gateway.properties";
	private static final Logger logger = LoggerFactory.getLogger(RuptelaGatewayConfiguration.class);

	static {
		initialize();
	}

	private RuptelaGatewayConfiguration() {
	}
	...
	private static URL resolveURL(String path) {
		...
		if (url == null) {
			url = RuptelaGatewayConfiguration.class.getClassLoader().getResource(path);
		}
		if (url == null) {
