 ...
 *
 */
public class HobbiesGatewayConfiguration implements PropertyKeys {

	private static Properties props;
	private static final String configFileName = "gateway.properties";
	private static final Logger logger = LoggerFactory.getLogger(HobbiesGatewayConfiguration.class);

	static {
		initialize();
	}

	private HobbiesGatewayConfiguration() {
	}
	...
	private static URL resolveURL(String path) {
		...
		if (url == null) {
			url = HobbiesGatewayConfiguration.class.getClassLoader().getResource(path);
		}
		if (url == null) {
