...
public class KafkaUtil {
	...
	public static int getNumberOfPartitions(String topic){
	...
	public static String findNewLeader(String a_oldLeader, String a_topic, int a_partition, int a_port) throws Exception {
		for (int i = 0; i < 3; i++) {
	...
	public static void main(String[] args) {
		try {
			...
			int port = 9092;
			SimpleConsumer consumer = createSimpleConsumer(brokers, port, topic, 0);
			ByteBufferMessageSet bufferMessageSet = consumeData(consumer,topic,0,port,2);
			readOffset = getLastOffset(consumer,topic,partition, kafka.api.OffsetRequest.EarliestTime(), clientName);
