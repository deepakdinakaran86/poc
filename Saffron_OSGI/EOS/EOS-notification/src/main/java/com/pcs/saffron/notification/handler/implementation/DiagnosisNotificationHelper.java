...
import com.pcs.saffron.notification.broker.kafka.publisher.KafkaDiagnosticPublisher;
...
public class DiagnosisNotificationHelper implements NotificationHandler{

	private static final Logger LOGGER = LoggerFactory.getLogger(DiagnosisNotificationHelper.class);
	...
	private CoreConsumer getConsumerByMode(DistributorMode mode,ConsumerType type){
		...
		switch(mode){
			case JMS:
				...
				return DiagnosisDistributionUtil.getJmsConsumerConfigured(coreConsumer);
			case KAFKA:
				coreConsumer =  new KafkaBaseConsumer().getConsumer(type);
				return DiagnosisDistributionUtil.getKafkaConsumerConfigured(coreConsumer, type);
			default:
				return null;
		}
	}
	...
	public CorePublisher getPublisher(DistributorMode mode){
		try{ 
			...
			switch(mode){
				...
				case KAFKA:
					corePublisher = (CorePublisher) new KafkaDiagnosticPublisher();
					return corePublisher;
				default:
					return null;
			}
	...
	public static CorePublisher getBasePublisher(DistributorMode mode){
		try{ 
			switch(mode){
				...
				case KAFKA:
					return (CorePublisher) new KafkaDiagnosticPublisher();
				default:
					return null;
			}
		}catch(Exception ex){
