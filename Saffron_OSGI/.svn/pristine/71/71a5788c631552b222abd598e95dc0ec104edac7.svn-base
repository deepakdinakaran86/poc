...
import com.pcs.device.gateway.hobbies.event.DeviceMessageEvent;
import com.pcs.device.gateway.hobbies.event.listener.adapter.DeviceMessageAdapter;
	...
	public void messageReady(DeviceMessageEvent deviceMessageEvent,
		...
		try {
			for (Message message : deviceMessageEvent.getMessages()) {
				DeviceManagerUtil.getHobbiesDeviceManager().notifyMessage(message, deviceConfiguration, MessageCompletion.PARTIAL, gateway);
			}
			
			if(MessageUtil.getDiagNotificationHandler() != null && HobbiesGatewayConfiguration.getProperty(HobbiesGatewayConfiguration.DIAG_ENABLE).equalsIgnoreCase("true")){
