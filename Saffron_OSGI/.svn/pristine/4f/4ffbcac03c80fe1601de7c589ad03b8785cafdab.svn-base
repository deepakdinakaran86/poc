package com.pcs.device.gateway.ruptela.event.handler;
...
import com.pcs.device.gateway.ruptela.event.listener.adapter.DeviceMessageAdapter;
	...
	public void messageReady(DeviceMessageEvent deviceMessageEvent,
		...
		try {
			for (Message message : deviceMessageEvent.getMessages()) {
				DeviceManagerUtil.getRuptelaDeviceManager().notifyMessage(message, deviceConfiguration, MessageCompletion.PARTIAL, gateway);
			}
			
			if(MessageUtil.getDiagNotificationHandler() != null && RuptelaGatewayConfiguration.getProperty(RuptelaGatewayConfiguration.DIAG_ENABLE).equalsIgnoreCase("true")){
