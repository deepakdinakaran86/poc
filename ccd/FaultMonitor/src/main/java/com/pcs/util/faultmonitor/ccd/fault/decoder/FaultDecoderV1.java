...
public class FaultDecoderV1 {
	...
	public static List<FaultData> decode(File faultFile) throws Exception{
		...
		if(!readLines.isEmpty()){
			...
			for (String line : readLines) {
				...
				case SPEED:
					state = DecoderState.SPEED;
					extractTimestamp(line, faultDetails);
					break;
	...
	private static void extractTimestamp(String data,FaultDataWrapper faultDetails) {
		...
		while (parameterRefinedToken.hasMoreElements()) {
			...
			faultSnapshot.addParameter(new FaultParameter(parameterDataToken.nextToken(),parameterDataToken.nextToken()));
		}
