...
import com.pcs.util.faultmonitor.util.GzipFaultFileUtil;

/**
 * @author pcseg171
 *
 */
public class FaultDecoderV3 {

	private static final Logger LOGGER = LoggerFactory.getLogger(FaultDecoderV3.class);
	 ...
	 * 
	 */
	private static final String colonDelimiter = ":",equalDelimiter="=",semicolonDelimiter=";",forwardSlash="/",spaceDelimiter=" ";
	...
	public static DeviceData decode(File faultFile) throws Exception{
		...
		if(GzipUtils.isCompressedFilename(faultFile.getName())){
			readLines = GzipFaultFileUtil.readZipFile(faultFile);
		}else{
			readLines = FileUtils.readLines(faultFile);
		}
		if(!readLines.isEmpty()){

			DecoderState state = DecoderState.INIT;
			DeviceData deviceData = new DeviceData();
			deviceData.setRealtimeData(true);
			for (String line : readLines) {
				...
				case VIDNUMBER:
					...
					extractSwId(line, deviceData);
					break;
				case SOFTWAREID:
					state = DecoderState.CALIBRATIONID;
					extractCaliberationId(line, deviceData);
					break;
				case CALIBRATIONID:
				...
				case SPEED:
					...
					extractColumnHeaders(line, deviceData);
					break;
				case SPN:
					state = DecoderState.SPN;
					extractParameters(line, deviceData);
					break;
				case SNAPSHOT:
					state = DecoderState.COMPLETE;
					break;
				case COMPLETE:
					LOGGER.info("File parsing complete",state);
					break;
				default:
	...
	/**
	 * This method mostly would not be used
	 * @param data
	 * @param deviceData
	 * @throws Exception
	 */
	private static void extractFaultData(String data,DeviceData deviceData) throws Exception {
		...
		String LSInfo = null;
		if(!faultDataToken.hasMoreTokens()){
			LOGGER.info("No more tokens found");
			return;
		}else{
			LSInfo = faultDataToken.nextToken();
			LOGGER.info("LSINFO {}",LSInfo);
			if(LSInfo == null || LSInfo.trim().length() <= 0){
				LOGGER.info("Empty data found");
				return;
			}
		}
	...
	private static void extractColumnHeaders(String data,DeviceData deviceData) {

		StringTokenizer snapshotToken = new StringTokenizer(data,semicolonDelimiter);
		...
		while(snapshotToken.hasMoreTokens()){
			String parameterName = snapshotToken.nextToken();
			deviceData.addParameterNames(parameterName);
			headers.put(index, parameterName);
			index++;
		}

	}
	private static void extractParameters(String data,DeviceData deviceData) {
		Snapshot snapshot = new Snapshot();
		StringTokenizer snapshotToken = new StringTokenizer(data,semicolonDelimiter);

		String timeInfo = snapshotToken.nextToken();
		StringTokenizer dateToken = new StringTokenizer(timeInfo,forwardSlash);
		
		
		

		if(!isValidToken(dateToken, 3))
			return;
		...
		StringTokenizer hourToken = new StringTokenizer(year,spaceDelimiter);
		year = hourToken.nextToken();
		
		String time = hourToken.nextToken();
		
		
		StringBuffer timeBuffer = new StringBuffer();
		
		timeBuffer.append(day).append("-").append(month).append("-").append(year).append(spaceDelimiter).append(time);
		snapshot.setTimestamp(timeBuffer.toString());

		int index = 1;
		while (snapshotToken.hasMoreElements()) {
			String parameterData =  snapshotToken.nextToken();
			snapshot.addParameter(new Parameter(headers.get(index),parameterData));
			index++;
		}
	...
	private static void extractSwId(String data,DeviceData deviceData) {
		...
		deviceData.setVidNumber(swIdToken.nextToken());
	}

	private static void extractCaliberationId(String data,DeviceData deviceData) {
		...
		deviceData.setVidNumber(caliberationIdToken.nextToken());
	}
