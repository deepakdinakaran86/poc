...
public class FaultResponseTester {
	
	public FaultResponseTester(){
		init();
	}
	...
	public static void init() {
		try{
			...
			coreConsumer.setTopic("fault_response_stream");
	...
	static class TestListener implements CoreListener{
		...
		public void consumeData(List<KafkaStream<byte[], byte[]>> streams) {
			...
			for(final KafkaStream stream : streams){
				System.out.println("Started consuming fault response .............");
				ExecutorService executor;
				executor = Executors.newFixedThreadPool(10);
				executor.submit(new FaultResponseMessageUtil(stream,i));
				i++;
			}
        }
