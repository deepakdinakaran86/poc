/**
 * Copyright 2014 Pacific Controls Software Services LLC (PCSS). All Rights
 * Reserved.
 *
 * This software is the property of Pacific Controls Software Services LLC and
 * its suppliers. The intellectual and technical concepts contained herein are
 * proprietary to PCSS. Dissemination of this information or reproduction of
 * this material is strictly forbidden unless prior written permission is
 * obtained from Pacific Controls Software Services.
 *
 * PCSS MAKES NO REPRESENTATION OR WARRANTIES ABOUT THE SUITABILITY OF THE
 * SOFTWARE, EITHER EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE IMPLIED
 * WARRANTIES OF MERCHANTANILITY, FITNESS FOR A PARTICULAR PURPOSE, OR
 * NON-INFRINGMENT. PCSS SHALL NOT BE LIABLE FOR ANY DAMAGES SUFFERED BY
 * LICENSEE AS A RESULT OF USING, MODIFYING OR DISTRIBUTING THIS SOFTWARE OR ITS
 * DERIVATIVES.
 */
package com.pcs.ccd.heartbeat.beans;

import java.io.Serializable;
import java.util.Map;
import java.util.UUID;

/**
 * EntityTemplateDTO
 *
 * @description DTO for Entity Template
 * @author Twinkle (pcseg297)
 * @date August 2014
 * @since galaxy-1.0.0
 */
public class EntityTemplateDTO implements Serializable {

	private static final long serialVersionUID = 2715521038150403960L;
	private UUID uuid;
	private DomainDTO domain;
	private String entityTemplateName;
	private Map<String, String> fieldValidation;
	private String globalEntityTemplateName;
	private String globalEntityType;
	private String htmlPage;
	private String identifierField;
	private Boolean isModifiable;
	private Boolean isSharable;
	private StatusDTO status;

	public UUID getUuid() {
		return uuid;
	}

	public void setUuid(UUID uuid) {
		this.uuid = uuid;
	}

	public DomainDTO getDomain() {
		return domain;
	}

	public void setDomain(DomainDTO domain) {
		this.domain = domain;
	}

	public String getEntityTemplateName() {
		return entityTemplateName;
	}

	public void setEntityTemplateName(String entityTemplateName) {
		this.entityTemplateName = entityTemplateName;
	}

	public Map<String, String> getFieldValidation() {
		return fieldValidation;
	}

	public void setFieldValidation(Map<String, String> fieldValidation) {
		this.fieldValidation = fieldValidation;
	}

	public String getGlobalEntityTemplateName() {
		return globalEntityTemplateName;
	}

	public void setGlobalEntityTemplateName(String globalEntityTemplateName) {
		this.globalEntityTemplateName = globalEntityTemplateName;
	}

	public String getGlobalEntityType() {
		return globalEntityType;
	}

	public void setGlobalEntityType(String globalEntityType) {
		this.globalEntityType = globalEntityType;
	}

	public String getHtmlPage() {
		return htmlPage;
	}

	public void setHtmlPage(String htmlPage) {
		this.htmlPage = htmlPage;
	}

	public String getIdentifierField() {
		return identifierField;
	}

	public void setIdentifierField(String identifierField) {
		this.identifierField = identifierField;
	}

	public Boolean getIsModifiable() {
		return isModifiable;
	}

	public void setIsModifiable(Boolean isModifiable) {
		this.isModifiable = isModifiable;
	}

	public Boolean getIsSharable() {
		return isSharable;
	}

	public void setIsSharable(Boolean isSharable) {
		this.isSharable = isSharable;
	}

	public StatusDTO getStatus() {
		return status;
	}

	public void setStatus(StatusDTO status) {
		this.status = status;
	}
}
