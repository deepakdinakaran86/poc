...
function FnResAHUList(response){
	...
	var ViewLink = (ObjPageAccess['view']=='1') ? "<a class='grid-chiller-view'>#=name#</a>" : "<a href='Javascript:void(0)'>#=name#</a>";
	var PointMapLink = (ObjPageAccess['mappoint']=='1') ? "<a class='grid-chiller-mappoints btn btn-raised default'>Map Points</a>" : "";
	...
	FnDrawGridView('#gapp-chiller-list',ArrResponseData,ArrColumns,ObjGridConfig);
	
	$("#gapp-chiller-list").data("kendoGrid").tbody.on("click", ".grid-chiller-view", function(e) {
		var tr = $(this).closest("tr");
		var data = $("#gapp-chiller-list").data('kendoGrid').dataItem(tr);
		data = data.identifier;
		//alert(data);
		$( "#equip_id").val(data);
		$("#gapp-chiller-view").submit();
	});
	
	$("#gapp-chiller-list").data("kendoGrid").tbody.on("click", ".grid-chiller-mappoints", function(e) {
		var tr = $(this).closest("tr");
		var data = $("#gapp-chiller-list").data('kendoGrid').dataItem(tr);
...
function FnSubmitEquipmentMapPoints(ObjEquipment){
	...
	$("#gapp-chiller-pointmapping").submit();
}

function FnCreateChiller(){
	$('#gapp-chiller-create').submit();
}
