...
$(document).ready(function(){
	...
	// Form Validation
	$("#gapp-chiller-form").kendoValidator({
										validateOnBlur : true,
	...
	if(VarEditEquipId != ''){
		FnGetPointsTag();
		FnGetChillerEquipmentDetails(VarEditEquipId);
	}
...
function FnGetChillerEquipmentDetails(VarEquipIdentifier){
	$("#gapp-chiller-form :input").prop("disabled", true);
	$('#gapp-chiller-save').attr('disabled',true);
	$("#gapp-chiller-cancel").prop("disabled", false);
	...
	FnMakeAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResAHUEquipmentDetails);
}

function FnResAHUEquipmentDetails(response){
	//console.log(response);
	var ArrResponse = response;
	if(!$.isEmptyObject(ArrResponse)){
		$.each(ArrResponse['fieldValues'],function(key,obj){
			if($('#gapp-chiller-form #'+obj['key'])){
				if(obj['key'] != 'rfrVFD' && obj['key'] != 'sfrVFD' && obj['key'] != 'ees'){
					if(obj['key'] == 'facilityName'){
						$("#facility option:contains('"+obj['value']+"')").attr('selected', true);
					} else {
						$('#gapp-chiller-form #'+obj['key']).val(obj['value']);
					}
				} else if(obj['key'] == 'rfrVFD'){
					if(obj['value'] != undefined && obj['value'] == 'true'){
						$('#gapp-chiller-form #'+obj['key']).attr('checked',true);							
						FnreturnFanRatingChecked();
					} else {
						$('#gapp-chiller-form #'+obj['key']).attr('checked',false);
					}
		...
		FnResAHUPointList(ArrPoints);
	}	
}

function FnResAHUPointList(ArrPoints){
	...
	FnDrawGridView('#gapp-chillerpoint-list',ArrPoints,ArrColumns,ObjGridConfig);
...
function FnGetEquipmentTemplate(){
	var ObjTemplate = {};
	ObjTemplate['entityTemplate'] = {"entityTemplateName" : "AvocadoAHUTemplate"};
	if(VarEditEquipId != ''){
...
function FnConstructSkySparkTags(){
	...
	ObjSkySparkTags['ahu'] = true;
	ObjSkySparkTags['fcu'] = false;
...
function FnGetSkySparkPointTags(){
	var ArrTags = [];
	var ArrData = $('#gapp-chillerpoint-list').data("kendoGrid").dataSource.data();
	//console.log(ArrData);
	//return false;
...
function FnSaveAHU(){
	$('#gapp-chiller-save, #gapp-chiller-cancel').attr('disabled',true);
	var validator = $("#gapp-chiller-form").data("kendoValidator");
	validator.hideMessages();
	//kendo.ui.progress($("#gapp-chiller-form"), true);
	$("#GBL_loading").show();
	if (validator.validate()) { //alert('success');
		if(VarEditEquipId == ''){ //Create chiller
			...
			VarParam['fieldValues'] = FnGetFormFieldValues("gapp-chiller-form","input[type=text],select,input[type=checkbox]");			
			VarParam['skySparkEquipmentTags'] = FnConstructSkySparkTags();
			//console.log(VarParam);
			
			FnMakeAsyncAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResSaveAHU);
			...
			//console.log(VarParam);
			//return false;
			FnMakeAsyncAjaxRequest(VarUrl, 'PUT', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResUpdateAHU);
			
		}
	} else {
		$('#gapp-chiller-save, #gapp-chiller-cancel').attr('disabled',false);
		//kendo.ui.progress($("#gapp-chiller-form"), false);
		$("#GBL_loading").hide();
...
function FnResSaveAHU(response){
	//console.log(response);
...
function FnResUpdateAHU(response){
	...
	if(ArrResponse.status == 'SUCCESS'){
		...
		//FnRedirect("/portal/equipments/chiller/list",2000);
		FnFormRedirect('gapp-chiller-list',GBLDelayTime);
...
function FnCancelChiller(){
	/*var VarFrmObj = document.getElementById('gapp-chiller-form');
	VarFrmObj.reset();
	var validator = $("#gapp-chiller-form").data("kendoValidator");
	validator.hideMessages();*/
	
	if(GBLcancel > 0){
		$('#GBLModalcancel #hiddencancelFrm').val('gapp-chiller-list');
		$('#GBLModalcancel').modal('show');
	} else {
		$('#gapp-chiller-list').submit();
	}
}

function FnNavigateAHUList(){
	$('#gapp-chiller-list').submit();
}

function FnEditChiller(){
	$("#gapp-chiller-form :input").prop("disabled", false);
	$("#gapp-chiller-save").prop("disabled", false);
	$('#equipmentId, #facility, #timeZone').prop("disabled", true);
	$('#gapp-chiller-edit').hide();
}
