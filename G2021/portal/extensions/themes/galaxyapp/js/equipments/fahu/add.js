...
$(document).ready(function(){
	...
	if(VarEditEquipId != ''){
		FnGetPointsTag();
		FnGetFAHUEquipmentDetails(VarEditEquipId);
	}
...
function FnGetFAHUEquipmentDetails(VarEquipIdentifier){
	$("#gapp-fahu-form :input").prop("disabled", true);
	$('#gapp-fahu-save').attr('disabled',true);
	$("#gapp-fahu-cancel").prop("disabled", false);
	...
	VarParam['entityTemplate'] = {"entityTemplateName" : "AvocadoFAHUTemplate"};
	VarParam['identifier'] = {"key": "identifier","value": VarEquipIdentifier};
	//console.log(VarParam);
	FnMakeAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResFAHUEquipmentDetails);
}

function FnResFAHUEquipmentDetails(response){
	//console.log(response);
	var ArrResponse = response;
	if(!$.isEmptyObject(ArrResponse)){
		$.each(ArrResponse['fieldValues'],function(key,obj){
			if($('#gapp-fahu-form #'+obj['key'])){
				if(obj['key'] != 'rfrVFD' && obj['key'] != 'sfrVFD' && obj['key'] != 'ees'){
					if(obj['key'] == 'facilityName'){
						$("#facility option:contains('"+obj['value']+"')").attr('selected', true);
					} else {
						$('#gapp-fahu-form #'+obj['key']).val(obj['value']);
					}
				} else if(obj['key'] == 'rfrVFD'){
					if(obj['value'] != undefined && obj['value'] == 'true'){
						$('#gapp-fahu-form #'+obj['key']).attr('checked',true);							
						FnreturnFanRatingChecked();
					} else {
						$('#gapp-fahu-form #'+obj['key']).attr('checked',false);
					}
		...
		FnResFAHUPointList(ArrPoints);
	}	
}

function FnResFAHUPointList(ArrPoints){
	...
	FnDrawGridView('#gapp-fahupoint-list',ArrPoints,ArrColumns,ObjGridConfig);
...
function FnGetEquipmentTemplate(){
	var ObjTemplate = {};
	ObjTemplate['entityTemplate'] = {"entityTemplateName" : "AvocadoFAHUTemplate"};
	if(VarEditEquipId != ''){
...
function FnGetSkySparkPointTags(){
	var ArrTags = [];
	var ArrData = $('#gapp-fahupoint-list').data("kendoGrid").dataSource.data();
	//console.log(ArrData);
	//return false;
...
function FnSaveFAHU(){
	$('#gapp-ahu-save, #gapp-fahu-cancel').attr('disabled',true);
	var validator = $("#gapp-fahu-form").data("kendoValidator");
	validator.hideMessages();
	//kendo.ui.progress($("#gapp-fahu-form"), true);
	$("#GBL_loading").show();
	if (validator.validate()) { //alert('success');
		if(VarEditEquipId == ''){ //Create AHU
			...
			VarParam['fieldValues'] = FnGetFormFieldValues("gapp-fahu-form","input[type=text],select,input[type=checkbox]");			
			VarParam['skySparkEquipmentTags'] = FnConstructSkySparkTags();
			//console.log(VarParam);
			
			FnMakeAsyncAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResSaveFAHU);
			...
			//console.log(VarParam);
			FnMakeAsyncAjaxRequest(VarUrl, 'PUT', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResUpdateFAHU);
			
		}
	} else {
		$('#gapp-fahu-save, #gapp-fahu-cancel').attr('disabled',false);
		//kendo.ui.progress($("#gapp-fahu-form"), false);
		$("#GBL_loading").hide();
...
function FnResSaveFAHU(response){
	//console.log(response);
...
function FnResUpdateFAHU(response){
	...
	if(ArrResponse.status == 'SUCCESS'){
		...
		//FnRedirect("/portal/equipments/fahu/list",2000);
		FnFormRedirect('gapp-fahu-list',GBLDelayTime);
...
function FnCancelFAHU(){
	/*var VarFrmObj = document.getElementById('gapp-fahu-form');
	VarFrmObj.reset();
	var validator = $("#gapp-fahu-form").data("kendoValidator");
	validator.hideMessages();*/
	
	if(GBLcancel > 0){
		$('#GBLModalcancel #hiddencancelFrm').val('gapp-fahu-list');
		$('#GBLModalcancel').modal('show');
	} else {
		$('#gapp-fahu-list').submit();
	}
}

function FnNavigateFAHUList(){
	$('#gapp-fahu-list').submit();
}

function FnEditFAHU(){
	$("#gapp-fahu-form :input").prop("disabled", false);
	$("#gapp-fahu-save").prop("disabled", false);
