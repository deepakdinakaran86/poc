...
function FnGetFAHUList(){
	...
	FnMakeAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResFAHUList);	
}

function FnResFAHUList(response){
	...
	var ViewLink = (ObjPageAccess['view']=='1') ? "<a class='grid-fahu-view'>#=name#</a>" : "<a href='Javascript:void(0)'>#=name#</a>";
	var PointMapLink = (ObjPageAccess['mappoint']=='1') ? "<a class='grid-fahu-mappoints btn btn-raised default'>Map Points</a>" : "";
	...
	FnDrawGridView('#gapp-fahu-list',ArrResponseData,ArrColumns,ObjGridConfig);
	
	$("#gapp-fahu-list").data("kendoGrid").tbody.on("click", ".grid-fahu-view", function(e) {
		var tr = $(this).closest("tr");
		var data = $("#gapp-fahu-list").data('kendoGrid').dataItem(tr);
		data = data.identifier;
		//alert(data);
		$( "#equip_id").val(data);
		$("#gapp-fahu-view").submit();
	});
	
	$("#gapp-fahu-list").data("kendoGrid").tbody.on("click", ".grid-fahu-mappoints", function(e) {
		var tr = $(this).closest("tr");
		var data = $("#gapp-fahu-list").data('kendoGrid').dataItem(tr);
...
function FnSubmitEquipmentMapPoints(ObjEquipment){
	...
	$("#gapp-fahu-pointmapping").submit();
}

function FnCreateFAHU(){
	$('#gapp-fahu-create').submit();
}
