...
$(document).ready(function(){
	...
	if(VarEditEquipId != ''){
		FnGetPointsTag();
		FnGetSUEquipmentDetails(VarEditEquipId);
	}
...
function FnGetSUEquipmentDetails(VarEquipIdentifier){
	$("#gapp-su-form :input").prop("disabled", true);
	$('#gapp-su-save').attr('disabled',true);
	$("#gapp-su-cancel").prop("disabled", false);
	...
	FnMakeAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResSUEquipmentDetails);
}

function FnResSUEquipmentDetails(response){
	//console.log(response);
	var ArrResponse = response;
	if(!$.isEmptyObject(ArrResponse)){
		$.each(ArrResponse['fieldValues'],function(key,obj){
			if($('#gapp-su-form #'+obj['key'])){
				if(obj['key'] != 'rfrVFD' && obj['key'] != 'sfrVFD' && obj['key'] != 'ees'){
					if(obj['key'] == 'facilityName'){
						$("#facility option:contains('"+obj['value']+"')").attr('selected', true);
					} else {
						$('#gapp-su-form #'+obj['key']).val(obj['value']);
					}
				} else if(obj['key'] == 'rfrVFD'){
					if(obj['value'] != undefined && obj['value'] == 'true'){
						$('#gapp-su-form #'+obj['key']).attr('checked',true);							
						FnreturnFanRatingChecked();
					} else {
						$('#gapp-su-form #'+obj['key']).attr('checked',false);
					}
		...
		FnResSUPointList(ArrPoints);
	}	
}

function FnResSUPointList(ArrPoints){
	...
	FnDrawGridView('#gapp-supoint-list',ArrPoints,ArrColumns,ObjGridConfig);
...
function FnGetEquipmentTemplate(){
	var ObjTemplate = {};
	ObjTemplate['entityTemplate'] = {"entityTemplateName" : "AvocadoSplitUnitTemplate"};
	if(VarEditEquipId != ''){
...
function FnGetSkySparkPointTags(){
	var ArrTags = [];
	var ArrData = $('#gapp-supoint-list').data("kendoGrid").dataSource.data();
	//console.log(ArrData);
	//return false;
...
function FnSaveSU(){
	$('#gapp-su-save, #gapp-su-cancel').attr('disabled',true);
	var validator = $("#gapp-su-form").data("kendoValidator");
	validator.hideMessages();
	//kendo.ui.progress($("#gapp-su-form"), true);
	$("#GBL_loading").show();
	if (validator.validate()) { //alert('success');
		if(VarEditEquipId == ''){ //Create SU
			...
			VarParam['fieldValues'] = FnGetFormFieldValues("gapp-su-form","input[type=text],select,input[type=checkbox]");			
			VarParam['skySparkEquipmentTags'] = FnConstructSkySparkTags();
			//console.log(VarParam);
			
			FnMakeAsyncAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResSaveSU);
			...
			//console.log(VarParam);
			FnMakeAsyncAjaxRequest(VarUrl, 'PUT', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResUpdateSU);
			
		}
	} else {
		$('#gapp-su-save, #gapp-su-cancel').attr('disabled',false);
		//kendo.ui.progress($("#gapp-su-form"), false);
		$("#GBL_loading").hide();
...
function FnResUpdateSU(response){
	...
	if(ArrResponse.status == 'SUCCESS'){
		...
		//FnRedirect("/portal/equipments/su/list",2000);
		FnFormRedirect('gapp-su-list',GBLDelayTime);
...
function FnCancelSU(){
	/*var VarFrmObj = document.getElementById('gapp-su-form');
	VarFrmObj.reset();
	var validator = $("#gapp-su-form").data("kendoValidator");
	validator.hideMessages();*/
	
	if(GBLcancel > 0){
		$('#GBLModalcancel #hiddencancelFrm').val('gapp-su-list');
...
function FnNavigateSUList(){
	$('#gapp-su-list').submit();
}

function FnEditSU(){
	$("#gapp-su-form :input").prop("disabled", false);
	$("#gapp-su-save").prop("disabled", false);
