...
function FnGetPUList(){
	...
	FnMakeAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResPUList);	
}

function FnResPUList(response){
	...
	var ViewLink = (ObjPageAccess['view']=='1') ? "<a class='grid-pu-view'>#=name#</a>" : "<a href='Javascript:void(0)'>#=name#</a>";
	var PointMapLink = (ObjPageAccess['mappoint']=='1') ? "<a class='grid-pu-mappoints btn btn-raised default'>Map Points</a>" : "";
	...
	FnDrawGridView('#gapp-pu-list',ArrResponseData,ArrColumns,ObjGridConfig);
	
	$("#gapp-pu-list").data("kendoGrid").tbody.on("click", ".grid-pu-view", function(e) {
		var tr = $(this).closest("tr");
		var data = $("#gapp-pu-list").data('kendoGrid').dataItem(tr);
	...
	$("#gapp-pu-list").data("kendoGrid").tbody.on("click", ".grid-pu-mappoints", function(e) {
		var tr = $(this).closest("tr");
		var data = $("#gapp-pu-list").data('kendoGrid').dataItem(tr);
...
function FnSubmitEquipmentMapPoints(ObjEquipment){
	...
	$("#gapp-pu-pointmapping").submit();
}

function FnCreatePU(){
	$('#gapp-pu-create').submit();
}
