...
function FnGetFCUList(){
	...
	FnMakeAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResFCUList);	
}

function FnResFCUList(response){
	...
	var ViewLink = (ObjPageAccess['view']=='1') ? "<a class='grid-fcu-view'>#=name#</a>" : "<a href='Javascript:void(0)'>#=name#</a>";
	var PointMapLink = (ObjPageAccess['mappoint']=='1') ? "<a class='grid-fcu-mappoints btn btn-raised default'>Map Points</a>" : "";
	...
	FnDrawGridView('#gapp-fcu-list',ArrResponseData,ArrColumns,ObjGridConfig);
	
	$("#gapp-fcu-list").data("kendoGrid").tbody.on("click", ".grid-fcu-view", function(e) {
		var tr = $(this).closest("tr");
		var data = $("#gapp-fcu-list").data('kendoGrid').dataItem(tr);
		data = data.identifier;
		//alert(data);
		$( "#equip_id").val(data);
		$("#gapp-fcu-view").submit();
	});
	
	$("#gapp-fcu-list").data("kendoGrid").tbody.on("click", ".grid-fcu-mappoints", function(e) {
		var tr = $(this).closest("tr");
		var data = $("#gapp-fcu-list").data('kendoGrid').dataItem(tr);
...
function FnSubmitEquipmentMapPoints(ObjEquipment){
	...
	$("#gapp-fcu-pointmapping").submit();
}

function FnCreateFCU(){
	$('#gapp-fcu-create').submit();
}
