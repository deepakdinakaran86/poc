...
(function($) {
	...
	$.fn.seatCharts = function (setup) {
		...
		var rowId=0;
		var id="device-torch";
		var name="device-torch-name";
		
		var combo = $("<select></select>").attr("id", id).attr("name", name);
		 combo.append("<option value='true'>ON</option>"+"<option value='false'>" + "OFF" + "</option>");
		
		console.log(combo);
		//do this for each map row
		$.each(settings.map, function(row, characters) {
			rowId++;
			var $row = $('<div title="row"></div>').addClass('seatCharts-row');
			...
			if (settings.naming.left) {
				...
				$row.append(' &nbsp&nbsp<input class="row-checkbox " name="check-row" type="checkbox" onClick="FnSelectRowCheckbox(this)" id="'+rowId+'" /> ');
