...
$(document).ready(function(){
	...
	if(VarEditEquipId != ''){
		FnGetPointsTag();
		FnGetPUEquipmentDetails(VarEditEquipId);
	}
...
function FnGetPUEquipmentDetails(VarEquipIdentifier){
	$("#gapp-pu-form :input").prop("disabled", true);
	$('#gapp-pu-save').attr('disabled',true);
	$("#gapp-pu-cancel").prop("disabled", false);
	...
	VarParam['entityTemplate'] = {"entityTemplateName" : "AvocadoPackageUnitTemplate"};
	VarParam['identifier'] = {"key": "identifier","value": VarEquipIdentifier};
	//console.log(VarParam);
	FnMakeAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResPUEquipmentDetails);
}

function FnResPUEquipmentDetails(response){
	//console.log(response);
	var ArrResponse = response;
	if(!$.isEmptyObject(ArrResponse)){
		$.each(ArrResponse['fieldValues'],function(key,obj){
			if($('#gapp-pu-form #'+obj['key'])){
				if(obj['key'] != 'rfrVFD' && obj['key'] != 'sfrVFD' && obj['key'] != 'ees'){
					if(obj['key'] == 'facilityName'){
						$("#facility option:contains('"+obj['value']+"')").attr('selected', true);
					} else {
						$('#gapp-pu-form #'+obj['key']).val(obj['value']);
					}
				} else if(obj['key'] == 'rfrVFD'){
					if(obj['value'] != undefined && obj['value'] == 'true'){
						$('#gapp-pu-form #'+obj['key']).attr('checked',true);							
						FnreturnFanRatingChecked();
					} else {
						$('#gapp-pu-form #'+obj['key']).attr('checked',false);
					}
		...
		FnResPUPointList(ArrPoints);
		
	}	
}

function FnResPUPointList(ArrPoints){
	...
	FnDrawGridView('#gapp-pupoint-list',ArrPoints,ArrColumns,ObjGridConfig);
...
function FnGetEquipmentTemplate(){
	var ObjTemplate = {};
	ObjTemplate['entityTemplate'] = {"entityTemplateName" : "AvocadoPackageUnitTemplate"};
	if(VarEditEquipId != ''){
...
function FnGetSkySparkPointTags(){
	var ArrTags = [];
	var ArrData = $('#gapp-pupoint-list').data("kendoGrid").dataSource.data();
	//console.log(ArrData);
	//return false;
...
function FnSavePU(){
	$('#gapp-pu-save, #gapp-pu-cancel').attr('disabled',true);
	var validator = $("#gapp-pu-form").data("kendoValidator");
	validator.hideMessages();
	//kendo.ui.progress($("#gapp-pu-form"), true);
	$("#GBL_loading").show();
	if (validator.validate()) { //alert('success');
		if(VarEditEquipId == ''){ //Create pu
			...
			VarParam['fieldValues'] = FnGetFormFieldValues("gapp-pu-form","input[type=text],select,input[type=checkbox]");			
			VarParam['skySparkEquipmentTags'] = FnConstructSkySparkTags();
			//console.log(VarParam);
			
			FnMakeAsyncAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResSavePU);
			...
			//console.log(VarParam);
			FnMakeAsyncAjaxRequest(VarUrl, 'PUT', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResUpdatePU);
			
		}
	} else {
		$('#gapp-pu-save, #gapp-pu-cancel').attr('disabled',false);
		//kendo.ui.progress($("#gapp-pu-form"), false);
		$("#GBL_loading").hide();
...
function FnResUpdatePU(response){
	...
	if(ArrResponse.status == 'SUCCESS'){
		...
		//FnRedirect("/portal/equipments/pu/list",2000);
		FnFormRedirect('gapp-pu-list',GBLDelayTime);
...
function FnCancelPU(){
	/*var VarFrmObj = document.getElementById('gapp-pu-form');
	VarFrmObj.reset();
	var validator = $("#gapp-pu-form").data("kendoValidator");
	validator.hideMessages();*/
	
	if(GBLcancel > 0){
		$('#GBLModalcancel #hiddencancelFrm').val('gapp-pu-list');
...
function FnNavigatePUList(){
	$('#gapp-pu-list').submit();
}


function FnEditPU(){
	$("#gapp-pu-form :input").prop("disabled", false);
	$("#gapp-pu-save").prop("disabled", false);
