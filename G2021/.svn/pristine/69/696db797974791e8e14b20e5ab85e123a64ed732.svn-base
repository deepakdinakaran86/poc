...
$(document).ready(function(){
	...
	if(VarEditEquipId != ''){
		FnGetPointsTag();
		FnGetAHUEquipmentDetails(VarEditEquipId);
	}
...
function FnGetAHUEquipmentDetails(VarEquipIdentifier){
	$("#gapp-ahu-form :input").prop("disabled", true);
	$('#gapp-ahu-save').attr('disabled',true);
	$("#gapp-ahu-cancel").prop("disabled", false);
...
function FnResAHUPointList(ArrPoints){
	...
	FnDrawGridView('#gapp-ahupoint-list',ArrPoints,ArrColumns,ObjGridConfig);
...
function FnGetSkySparkPointTags(){
	var ArrTags = [];
	var ArrData = $('#gapp-ahupoint-list').data("kendoGrid").dataSource.data();
	//console.log(ArrData);
	//return false;
...
function FnSaveAHU(){
	$('#gapp-ahu-save, #gapp-ahu-cancel').attr('disabled',true);
	var validator = $("#gapp-ahu-form").data("kendoValidator");
	validator.hideMessages();
	//kendo.ui.progress($("#gapp-ahu-form"), true);
	$("#GBL_loading").show();
	if (validator.validate()) { //alert('success');
		if(VarEditEquipId == ''){ //Create AHU
			...
			VarParam['fieldValues'] = FnGetFormFieldValues("gapp-ahu-form","input[type=text],select,input[type=checkbox]");			
		...
		$('#gapp-ahu-save, #gapp-ahu-cancel').attr('disabled',false);
		//kendo.ui.progress($("#gapp-ahu-form"), false);
		$("#GBL_loading").hide();
...
function FnResUpdateAHU(response){
	...
	if(ArrResponse.status == 'SUCCESS'){
		...
		//FnRedirect("/portal/equipments/ahu/list",2000);
		FnFormRedirect('gapp-ahu-list',GBLDelayTime);
...
function FnCancelAHU(){
	/*var VarFrmObj = document.getElementById('gapp-ahu-form');
	VarFrmObj.reset();
	var validator = $("#gapp-ahu-form").data("kendoValidator");
	validator.hideMessages();*/
	
	if(GBLcancel > 0){
		$('#GBLModalcancel #hiddencancelFrm').val('gapp-ahu-list');
		$('#GBLModalcancel').modal('show');
	} else {
		$('#gapp-ahu-list').submit();
	}
}
...
function FnEditAHU(){
	$("#gapp-ahu-form :input").prop("disabled", false);
	$("#gapp-ahu-save").prop("disabled", false);
