...
$(document).ready(function(){
	...
	map.on('click', function(e) {
		...
		if(GblGeoType === 'Circle'){
			...
			if(ArrPolyMarkers.length == 0){
				...
				var VarIcon = new LeafIcon({iconUrl: GblAppContextPath+'/plugins/leaflet/marker/geo-marker.png'});*/
			
				var geomarker = L.marker(e.latlng,{draggable: true}).addTo(map);
											...
											rules : {
												available:function(input){
													...
													if (typeof validate !== 'undefined' && validate !== false) {
														...
														var VarParam = {"domain": {"domainName": VarCurrentTenantInfo.tenantDomain},"fieldValues": [{"key": "geofenceName","value": FnTrim(input.val())}]};
														$.ajax({
															type:'POST',
															cache: true,
															async: false,
	...
	/*$("table").on("mouseover","tr.geofence_grid", function(e) {
	
		var tr = $(this).closest("tr");
		var VarGridId = (GblGeoType==='Polygon') ? 'draw-polygon-grid' : 'draw-roadmap-grid';
		var data = $("#"+VarGridId).data('kendoGrid').dataItem(tr);
		FnHighlightMarker(data,1);
		
	}).on("mouseout","tr.geofence_grid", function(e) {
	
		var tr = $(this).closest("tr");
		var VarGridId = (GblGeoType==='Polygon') ? 'draw-polygon-grid' : 'draw-roadmap-grid';
		var data = $("#"+VarGridId).data('kendoGrid').dataItem(tr);
		FnHighlightMarker(data,0);
		
	});*/
	
	if(VarEditGeofenceId != ''){
		FnGetGeofenceDetails(VarEditGeofenceId);
	}
...
function FnHighlightMarker(ObjMarker){
	...
	if(VarMarker.isBouncing() == false){
		VarMarker.setBouncingOptions({bounceHeight : 60,bounceSpeed  : 52,exclusive    : true});
		VarMarker.toggleBouncing();
	}
}
...
function FnSaveGeofence(){
	...
	if (validator.validate()) {
		...
		if(VarEditGeofenceId == ''){
			...
			var VarUrl = GblAppContextPath + '/ajax' + VarCreateGeofenceUrl;
...
function FnGetGeofenceDetails(VarGeofenceId){
	...
	FnMakeAjaxRequest(VarUrl, 'POST', JSON.stringify(VarParam), 'application/json; charset=utf-8', 'json', FnResGeofenceDetails);
}
...
function FnDrawGeofence(ArrCoords){
	if(GblGeoType === 'Circle'){
		...
		map.fitBounds(originPolyline.getBounds());
	}
